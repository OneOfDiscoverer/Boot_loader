...
void Modbus_Config(uint32_t baud)
{
	...
	GPIOA -> MODER &= ~(GPIO_MODER_MODER9 | GPIO_MODER_MODER10 |GPIO_MODER_MODER12);			//pa9 - rx pa10 - tx;
	...
	USART1 -> CR2 = 0;
	USART1 -> CR3 = 0;
	USART1 -> CR3 = USART_CR3_DMAR|USART_CR3_DMAT | USART_CR3_DEM; //;											// будем использовать RTS как DE
	USART1 -> RTOR = 25;
	USART1 -> CR1 = USART_CR1_DEAT | USART_CR1_DEDT ;// | USART_CR1_RTOIE;		// прерывание по таймауту приемника
...
//			DMA1_Channel3 -> CCR &= ~DMA_CCR_EN;
		...
		case MB_STATE_READY:
		{
			...
			{
				if ((mb.buf[0] == dev_addr) || (mb.buf[0] == 0x7f))
				{
					//ext_mb_packets++;
					Set_link_cnt(100,1);
					Modbus_parse();
			...
			{
				ext_mb_crc_err++;
//				for(uint16_t i = 0; i < MB_PACKET_SIZE; i++) mb.buf[i] = 0;
				mb.state = MB_STATE_START_WAIT;
			}
			break;
		}
